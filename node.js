...
app.post("/register", async (req, res) => {
  ...
  if (sunique && tunique) {
    if (Desig === "student") {
      await connection.insertStudent(
        First_name,
        Last_name,
        Birth_date,
        Phone,
      ...
      await connection.insertTeacher(
        First_name,
        Last_name,
        Birth_date,
        Phone,
...
app.post("/login", async (req, res) => {
  ...
  if (sAvailable) {
    ...
    if (tAvailable) {
      let data_t = await connection.getTeacherData({
        Email: email,
        Password: password,
      });
      let data_s = await connection.getAllStudentData();
...
app.post("/teacher_pic", storage.parser.single("img"), async (req, res) => {
  const result = await connection.updateTeacherProfPic(
    req.session.user_id,
    req.file.path
  );
  const data = await connection.getData(req.session.user_id);
